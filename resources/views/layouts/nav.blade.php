<nav id="main-nav" class="lg:bg-gray-50 lg:shadow-sm md:bg-transparent md:shadow-none py-1">
      ...
      <div class="hidden sm:flex sm:items-center sm:justify-between w-full">
        ...
        <div class="hidden sm:block sm:ml-6">
          <ul class="items-center justify-between space-x-6 hidden lg:flex ">
            ...
            <li>
              <a href="#" class="inline-flex gap-2 items-center justify-center w-full h-12 px-6 font-medium tracking-wide text-secondary transition duration-200 rounded  hover:text-accent_wine focus:shadow-outline focus:outline-none" aria-label="Compte" title="Compte">
  ...
  <div id="mobile-menu" class="transition duration-300 hidden lg:hidden from-accent_wine to-main bg-gradient-to-t absolute top-0 left-0 w-full">
    <div class="space-y-7 flex flex-col items-center justify-center mt-20 pb-10">
      ...
      <a href="{{ route('celliers.index') }}" class="justify-center font-light tracking-wide text-xl text-main transition-colors duration-200 hover:text-section_title" aria-label="celliers" title="celliers">
        Mes Celliers
      </a>
      <a href="{{ route('home') }}" class="justify-center font-light tracking-wide text-xl text-main transition-colors duration-200 hover:text-section_title" aria-label="articles" title="articles">
        Ajouter Cellier
      </a>
      <a href="{{ route('bouteilles') }}" class="justify-center font-light tracking-wide text-xl text-main transition-colors duration-200 hover:text-section_title" aria-label="saq" title="saq">
        Ajouter Bouteille
      </a>
      <a href="{{ route('logout') }}" class="justify-center font-light tracking-wide text-xl text-main transition-colors duration-200 hover:text-gray-500" aria-label="Sign out" title="deconnecter" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
        Déconnexion
        <form id="logout-form" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
      </a>
      <a href="#" class="pt-2 flex flex-col items-center justify-center text-xl rounded h-12 px-6 font-light tracking-wide text-secondary transition duration-200 hover:text-main" aria-label="Compte" title="Compte"><img class="max-w-[45px]" src="{{ asset('img/svg/user-gold.svg') }}" alt="user-profile">
        {{ Auth::user()->nom }} <!--  Affichage du nom Utilisateur. À modifier   -->
      </a>
